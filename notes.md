...
5. When functions are defined inside effects, they do not always reflect latest signal's data. Hence always declare functions outside them, like class members and they have no problem. Example: `src/app/pages/telecallers/bookings/bookings.ts` ###NOTES:1 ###NOTES:2
6. `src/app/pages/telecallers/bookings/bookings.ts` ###NOTES:3 - Custom renderer gets killed when using cells callback in grid settings. So, use `renderer` property and reassign to custom rendered once again in `cells` callback
