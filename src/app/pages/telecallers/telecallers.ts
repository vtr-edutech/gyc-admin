...
export class Telecallers {
  ...
  toggleTelecallerForm(toggle: boolean, mode: "edit" | "new", selectedTelecaller?: AdminUser<"telecaller">) {
  ...
  createTelecaller(form: NgForm) {
    if (form.invalid) {
      this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Please check form for errors' });
      return
    };
  ...
  updateTelecaller(form: NgForm) {
    ...
    if (!selectedTelecallerId) {
      this.messageService.add({ severity: 'error', summary: 'Error', detail: 'No telecaller selected' });
      return
    };
    this.telecallerService.updateTelecaller(selectedTelecallerId, form.value, () => {
      this.modalControls.isTelecallerModalOpen = false;
      this.modalControls.selectedTelecaller = null;
      this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Telecaller updated successfully' });
      form.reset();
      form.resetForm();
    }, (error) => {
      this.messageService.add({ severity: 'error', summary: 'Error', detail: error });
