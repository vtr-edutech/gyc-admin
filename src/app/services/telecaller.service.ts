...
export class TelecallerService {
  telecallers: WritableSignal<FetchState<AdminUser<'telecaller'>[]>> = signal<
  ...
  createTelecallerUserMeta: WritableSignal<FetchState<AdminUser<'telecaller'>>> = signal<
    FetchState<AdminUser<'telecaller'>>
  >({
    isLoading: false,
    error: null,
  ...
  createTelecaller(
    data: CreateTelecallerPayload,
    onSuccess?: Function,
    onError?: ErrorFnCallback,
  ): void {
    this.createTelecallerUserMeta.set({ isLoading: true, error: null, data: null });
    this.http
      .post<GenericResponse<AdminUser<'telecaller'>>>(API.CREATE_TELECALLER, data)
      .subscribe({
        next: (response) => {
        ...
        error: (error: HttpErrorResponse) => {
          this.createTelecallerUserMeta.set({
            isLoading: false,
            error: getErrorMessage(error),
            data: null,
          });
  ...
  updateTelecaller(
    ...
    this.createTelecallerUserMeta.set({ isLoading: true, error: null, data: null });
    this.http
      .post<GenericResponse<AdminUser<'telecaller'>>>(API.UPDATE_TELECALLER(id), data)
      .subscribe({
        next: (response) => {
          this.createTelecallerUserMeta.set({ isLoading: false, error: null, data: response });
          this.fetchTelecaller();
